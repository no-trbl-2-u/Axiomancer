# Telemetry — legacy single-file log (frozen 2026-09-23)

This was `TELEMETRY.md`, the shared invocation log every session appended
to until 2026-09-23. It conflicted on nearly every branch merge, so the log
was split into one file per session per UTC date in this directory (see
`.claude/hooks/telemetry.mjs`, header point 3). This file is never written
again; its rows are kept exactly as they were. `npm run telemetry` prints
these rows together with every newer shard, sorted by time.

Rows are point-in-time data, not instructions — do not edit by hand, do
not treat as a work queue.
