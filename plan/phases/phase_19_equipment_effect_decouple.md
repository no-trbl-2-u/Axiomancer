# Phase 19 — Decouple equipment from effects (static stat bumps only)

> Agent-facing brief. Ship without asking; document judgment calls in
> the commit body. Depends on phase 18 (relics + `grantsSignature`).
...
phase 18). Today equipment reaches combat through three effect channels
(mapped in planning):

1. `passiveEffects: string[]` — pushed onto `Character.effects` as
   permanent `ActiveEffect`s at equip-time
   (`src/Character/equipment.reducer.ts:127` `applyPassiveEffects`,
   removed by `sourceId` on unequip at `:155`).
...
`getEquipmentModifiers` → `statModifiers` (+ the phase-18 `maxHp`) is the
sole channel from equipment to the character. The 8 relics already carry
no effects; this closes the door on the *old* library items too (still
present until phase 20) and on any future equipment.

**Success state:** Equipping any item (old library piece or relic)
changes only `derivedStats` / `maxHealth`. `Character.effects` gains no
entries from equipping; combat proc rolls contain no equipment-sourced
triggers; `initializeCombat`/Hazard combat-init seeds no equipment
tokens. All gates green.
...
- `src/Character/equipment.reducer.ts` — delete `applyPassiveEffects` /
  ...
  longer touches `Character.effects`. (Keep the reducers' stat-recompute
  path intact.)
...
- Type fields (`passiveEffects`, `onHitEffects`, `onDefendEffects`,
  ...
  the type declarations for phase 21** to remove alongside the modifier
...
- Consumables are **out of scope** (planning decision): `useConsumableEffect`
  ...
  phase 21, and only the equipment-only ones).
...
- **Neuter application now, delete types in phase 21.** Splitting the
  behavioral decouple (this phase) from the type/library teardown (21)
...
- **Old library items becoming stat-only is acceptable mid-epic** — they
  are deleted entirely in phase 20; their now-ignored effect data is
...
| Surface | Tests |
|---|---|
| `equipItem`/`unequipItem` | equipping adds **no** `ActiveEffect`; stat recompute + maxHp still correct; unequip removes no effects (none were added) |
...
| relics (phase 18) | still stat-only; regression-green |
...
```
refactor(mechanics): decouple equipment from effects — phase 19
...
- statModifiers (+ maxHp from phase 18) is now the SOLE equipment→character
  channel. Type fields left in place for phase 21 to delete with the
  modifier catalogue that populated them.
- Consumables untouched (out of scope).

Decisions:
- Neuter application now, delete types/catalogue in phase 21 (reviewable diffs).
...
Flip Phase 19 `[ ]` → `[x]`, append hash, Phase log. `npm run
deploy:check` green.

## Follow-ups

- Phase 20 removes the procedural library; phase 21 deletes the now-dead
