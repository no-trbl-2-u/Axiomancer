// Axiomancer: Table Edition — print-and-play kit generator, KIT v3
// v3 additions (2026-07-22, bus-ride batch): Common Ground (universal sig, PROGRESS/Accord),
// 9 MINION cards (1 per enemy per tier — auto-play on reveal, phase-keyed turn-start effects,
// own HP), 9 ALLY cards (befriended enemies — Paid any die, exhaust for phase effect, absorb
// one entire strike -> exiled). All v3 content prints on its own trailing pages so a sleeved
// v2 kit upgrades by printing just those pages.
...
// turn); trigger the current phase's line at the start of each enemy turn; own HP, direct
// damage only. Phase = the tier printed on the current telegraph; Last Stand locks FURY.
const MINIONS = [
  { enemy: 'THE SKULK', tier: 'APPROACH', nm: 'Skulk Whelp', hp: 2, art: 'paw',
    etb: 'Attack 1.', A: 'Attack 1.', P: 'Attack 1, twice.', F: 'Attack 2, twice.' },
  { enemy: 'THE SKULK', tier: 'PRESS', nm: 'Filch-Shade', hp: 3, art: 'robber-hand',
    etb: 'You lose 1◆.', A: 'The Skulk gains GUARD 1.', P: 'One of your unspent dice becomes a MISS.', F: 'Attack 3.' },
  { enemy: 'THE SKULK', tier: 'FURY', nm: 'Night Chorus', hp: 2, art: 'raven',
    etb: 'Attack 2.', A: 'The Skulk gains GUARD 2.', P: 'Attack 2.', F: "The Skulk's attacks get +1 (stacks with Enrage)." },
  { enemy: 'THE SHELLBACK', tier: 'APPROACH', nm: 'Barnacle Cluster', hp: 4, art: 'triple-shells',
    etb: 'The Shellback gains GUARD 2.', A: 'It gains GUARD 1.', P: 'It gains GUARD 2.', F: 'It gains THORNS 1.' },
  { enemy: 'THE SHELLBACK', tier: 'PRESS', nm: 'Molt-Tender', hp: 3, art: 'crab',
    etb: 'Remove 1 of its Blight.', A: 'Remove 1 Blight.', P: 'Remove 1 Blight; it HEALS 1.', F: 'It HEALS 3.' },
  { enemy: 'THE SHELLBACK', tier: 'FURY', nm: 'Shard of Shell', hp: 5, art: 'broken-shield',
    etb: 'Nothing. It stands.', A: 'Its attackers take 1.', P: 'Attack 2.', F: 'Attack 4.' },
  { enemy: 'THE BRUTE', tier: 'APPROACH', nm: 'Scavenger Rat', hp: 2, art: 'mouse',
    etb: 'Attack 1.', A: 'The Brute HEALS 1.', P: 'Attack 1, twice.', F: 'Attack 2, twice.' },
  { enemy: 'THE BRUTE', tier: 'PRESS', nm: 'Drumbeater', hp: 3, art: 'drum',
...
// ANY color die; max 1 in play; refresh at your turn start; EXHAUST for the current phase's
// line; any time, assign one ENTIRE enemy strike to it — then exile it for the fight.
const ALLIES = [
  { nm: 'The Penitent Skulk', of: 'SKULK', built: 'MALISON', stance: 'mind', art: 'kneeling',
    A: 'SCRY 2 the enemy deck.', P: 'Your HEXES go on top this turn.', F: 'Deal 1 damage per fired Curse.' },
  { nm: 'Skulk of the Shallows', of: 'SKULK', built: 'TORRENT', stance: 'heart', art: 'high-tide',
    A: 'Advance your chain 1 (any color).', P: 'Your chain cannot break this turn.', F: 'Your next Surge Burst this turn deals 3.' },
  { nm: 'The Quiet Skulk', of: 'SKULK', built: 'STANDSTILL', stance: 'mind', art: 'mute',
    A: 'STAGGER 1.', P: 'STAGGER 2.', F: 'Deal 2 damage if the telegraph is at 0.' },
  { nm: 'The Doorwright', of: 'SHELLBACK', built: 'BASTION', stance: 'body', art: 'closed-doors',
    A: 'GUARD 2.', P: 'GUARD 2 (persists).', F: 'THORNS 1.' },
  { nm: 'Blightshell', of: 'SHELLBACK', built: 'CONTAGION', stance: 'body', art: 'turtle-shell',
    A: 'BLIGHT 1.', P: 'BLIGHT 2.', F: "This round's Blight tick doesn't reduce Blight." },
  { nm: 'The Kiln-Back', of: 'SHELLBACK', built: 'FOUNDRY', stance: 'body', art: 'furnace',
    A: 'TEMPER 1.', P: 'TEMPER 2.', F: 'KINDLE 1.' },
  { nm: 'The Gospel Brute', of: 'BRUTE', built: 'INVOCATION', stance: 'heart', art: 'prayer',
    A: 'CONVICTION +1.', P: 'ATTUNE 2.', F: 'CONVICTION +2.' },
  { nm: 'The Load-Bearer', of: 'BRUTE', built: 'BASTION', stance: 'body', art: 'weight-lifting-up',
    A: 'GUARD 3.', P: 'THORNS 1.', F: 'GUARD 4.' },
  { nm: 'The Sledge', of: 'BRUTE', built: 'FOUNDRY', stance: 'body', art: 'warhammer',
    A: 'TEMPER 1.', P: 'Deal 2 damage.', F: 'Deal 4 damage.' },
];
...
// ---------------------------------------------------------------- v3 PAGES
// Everything below is NEW in v3 and prints on its own trailing pages: print
// just these to upgrade a sleeved v2 kit.
function minionCell(m) {
  const rows = [['APPROACH', m.A], ['PRESS', m.P], ['FURY', m.F]].map(([t, fx]) =>
    `<div class="ph"><span class="pht">${t}</span><span class="phx">${boldKw(fx)}</span></div>`).join('');
  return `<div class="card foe" style="--c:${S.foe}">
    <div class="rail"><span class="rvert">${esc(m.enemy)} · MINION</span></div>
    <div class="bd">
      <div class="top">
        <div class="cfree">${icon(m.art, 'glbig')}</div>
        <div class="crarity">HP ${m.hp} · ${m.tier}</div>
      </div>
      <div class="cbtm" style="margin-top:3px">
        <div class="cname">${esc(m.nm)}</div>
        <div class="crule"></div>
...
function allyCell(a) {
  const rows = [['APPROACH', a.A], ['PRESS', a.P], ['FURY', a.F]].map(([t, fx]) =>
    `<div class="ph"><span class="pht">${t}</span><span class="phx">${boldKw(fx)}</span></div>`).join('');
  return `<div class="card sig" style="--c:${S[a.stance]}">
    <div class="rail"><span class="rvert">ALLY · PAY ANY DIE</span></div>
    <div class="bd">
      <div class="top">
...
const v3cells = [];
for (let i = 1; i <= 2; i++)
  v3cells.push(face(S.grey, 'GREY · SIGNATURE', {
    rarity: `UNIVERSAL ${i}/2`, freeIcon: `<span class="bigcost">5◆</span>`, freeVal: '',
    art: icon('shaking-hands', 'art-ic'), name: 'Common Ground',
    paidHtml: `<div class="cpaid">${CUBE}<span>${boldKw('Place 1 PROGRESS on the enemy. If it ends its turn with 3+ PROGRESS (boss: 5): Accord — the fight ties; it may join you.')}</span></div>`,
...
const ref = `
<div class="page ref">
  <h1>Axiomancer: Table Edition — Reference (P1 prototype · kit v3)</h1>
  ...
  Foundry — <b>White Heat</b> needs only its own die (4 dmg, 6 on SPECIAL).</p>
...
const html = `<!doctype html><html><head><meta charset="utf-8"><title>Axiomancer Table Edition — Print Kit v3</title>
<style>
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  .etb { font-size: 7.6pt; line-height: 1.25; margin-bottom: 3px; }
  .etb b { font-family: 'Arial Narrow', Consolas, sans-serif; font-size: 7.4pt; color: var(--c); text-transform: uppercase; }
  .phz { display: flex; flex-direction: column; gap: 2px; }
  .ph { display: grid; grid-template-columns: 0.62in 1fr; gap: 4px; align-items: start; }
  .pht { font-family: Consolas, monospace; font-size: 6.4pt; font-weight: bold; letter-spacing: 0.06em;
    border: 1px solid #999; padding: 0.5px 2px; text-align: center; }
  .phx { font-size: 7.4pt; line-height: 1.22; }
  .phx b { font-family: 'Arial Narrow', Consolas, sans-serif; font-size: 7.2pt; color: var(--c); text-transform: uppercase; }
  .mininote { font-size: 6.2pt; line-height: 1.25; color: #555; margin-top: 3px; font-style: italic; }
</style></head><body>${ref}${pages.join('')}${enemyPage}</body></html>`;

writeFileSync(new URL('./cards-v3.html', import.meta.url), html);
console.log(`cards: ${cells.length + v3cells.length} (${v3cells.length} new in v3) · card pages: ${pages.length} · total pages: ${pages.length + 3}`);
console.log(`v3-only card pages: ${v3start}..${v3end} of the card-page run (PDF pages ${v3start + 2}..${v3end + 2})`);
