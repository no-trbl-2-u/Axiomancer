 ...
 *   - Fallacy / Paradox tokens are generated when the player uses a card
 *     of that category.
...
/**
 * Adds 1 token of the philosophical resource matching the card's category
 * (Fallacy or Paradox). Called after a card resolves successfully.
 */
...
/**
 * Returns the philosophical category the card should generate on use.
 * Tier 1 / 2 cards generate their own category (Fallacy → Fallacy, Paradox
 * → Paradox). Tier 3 cards already consumed a philosophical token to fire,
 * so they generate the OPPOSING type — Fallacy → Paradox, Paradox → Fallacy.
 * This drives the late-combat card chain described in Spec 04b.
 */
export function philosophicalCategoryFor(card: Card): CardCategory {
    if (card.tier === 3) {
        return card.category === 'fallacy' ? 'paradox' : 'fallacy';
    }
 ...
 *   4. Generate one Fallacy or Paradox token from the card's category.
 *
...
export function executeCard(
    ...
    const genCategory = philosophicalCategoryFor(card);
