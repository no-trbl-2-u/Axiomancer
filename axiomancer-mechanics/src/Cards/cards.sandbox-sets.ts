...
// ── Spec 33 §4 — the per-theme dice-interaction valves (`dice-valves-33`) ────
// Every non-Forge theme gains exactly ONE dice-interaction card (spec 33 §4
// valve 3: reroll / convert / bank — "tap"), in its own voice, using ONLY the
// existing engine die-verbs (reroll_spent / convert_die_color / refresh_die /
// bank_spent_die — all combat-engine owned, no new mechanic kinds). Plus the
// FORGE special-amplifier enchantment (spec 33 §6): it raises the FIRED special
// payload (+1◆) WITHOUT changing what a special does (owner-lock D1), wired by
// card id at combat.engine.ts's `special-fired` hook. Sandbox-first: these are
// NEW cards (power added), so they stage here for the promotion pass — the
// preset recipe (5/5/5) is spec-locked, so slotting them into the library/
// presets is a structural decision deferred past D4 (see the phase report's
// needs-user-call). Pricing uses the D4-repriced die-verbs (cards.pricing.ts);
// each card carries its scoreCard arithmetic.
...
const mastersStamp: Card = {
    ...
    addedIn: '2026-07-17',
    tags: ['forge', 'enchantment', 'special', 'dice'],
};
...
export const SANDBOX_CARD_SETS: Record<string, SandboxCardSet> = {
    'dice-valves-33': {
        id: 'dice-valves-33',
        name: 'Spec 33 dice-interaction valves + FORGE special-amplifier',
        description:
            'Spec 33 §4 — one dice-interaction card per non-Forge theme '
            + '(reroll/convert/refresh/bank, in each theme\'s voice, existing '
            + 'engine die-verbs only) + the FORGE special-amplifier enchant '
            + '(Master\'s Stamp: +1◆ per fired special, spec 33 §6, wired at the '
            + 'special-fired hook). Sandbox-staged (NEW cards): promotion into '
            + 'the 5/5/5 preset recipe is a structural decision deferred past D4. '
            + 'Gate: each valve reaches the reroll/convert affordance D3-F3 '
            + 'flagged as absent, neither dead nor dominant, and Master\'s Stamp '
            + 'lifts specials-driven ◆ income without breaking the economy band.',
        cards: [
            recurringSymptom, breakTheTempo, secondTake, bleedForIt, bankTheYield,
            holdTheLine, restateThePoint, secondSight, changeOfHeart, mastersStamp,
        ],
    },
    'chooseX-vein': {
        id: 'chooseX-vein',
