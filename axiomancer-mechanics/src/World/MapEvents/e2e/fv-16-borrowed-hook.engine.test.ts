/**
 * fv-16 "The Borrowed Hook" — hermetic coverage for the first of S-01's
...
function fv16Payload(): NarrationPayload {
    ...
    const next: GameState = {
        ...state,
        world: { ...state.world, currentMap: { ...state.world.currentMap, currentNode: 'fv-16', consumedNodes: [] } },
    };
    const result = resolveMapEvent(next);
    if (result.event.kind !== 'narration') throw new Error('fv-16 did not resolve to a narration event');
    return { kind: 'narration', dialogue: result.event.dialogue };
}

describe('fv-16 "The Borrowed Hook"', () => {
    it('is wired as a narration node carrying the borrowed-hook dialogue tree', () => {
        const payload = fv16Payload();
        expect(payload.kind).toBe('narration');
        expect(payload.dialogue.id).toBe('fv-borrowed-hook');
    ...
    it.each([
        ['kept', 'boy-kept-the-hook'],
        ['left', 'boy-left-the-hook'],
        ['reported', 'boy-reported-the-hook'],
    ...
    it('sits in a strictly earlier column than every post-boss reader (S-01\'s binding placement rule)', () => {
        const def = getMapDefinition('coastal-continent', 'fishing-village');
        const hookColumn = def.nodes.find(n => n.id === 'fv-16')!.location[0];
        for (const readerId of ['fv-18', 'fv-7', 'fv-19']) {
            const readerColumn = def.nodes.find(n => n.id === readerId)!.location[0];
            expect(hookColumn, `fv-16 must precede ${readerId}`).toBeLessThan(readerColumn);
        }
    });
});
