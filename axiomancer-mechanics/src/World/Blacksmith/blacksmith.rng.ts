/**
 * Deterministic RNG for the Blacksmith encounter engine (mulberry32).
 *
 * Sibling copy of the Hazard RNG (`World/Hazard/hazard.rng.ts`) so the
 * `World/Blacksmith/` directory stays self-contained. The engine threads explicit RNG state
 * through the session; it NEVER calls Math.random directly. The blacksmith's
 * upgrade transitions are deterministic (no rolls), so the state is threaded
 * but stays untouched — held for parity with the other minigame engines and
 * any future variance (a critical-hone table, a haggle roll).
 */

import { seedInputToUint32, type SeedInput } from '../seed';

export interface BlacksmithRngState {
    /** mulberry32 internal state word (uint32). */
    s: number;
}

export function seedRng(seed: SeedInput): BlacksmithRngState {
    let s = seedInputToUint32(seed);
    s = (s + 0x9e3779b9) >>> 0;
    return { s };
}

/** Returns a float in [0, 1) plus the advanced state. Pure. */
export function nextFloat(state: BlacksmithRngState): { value: number; state: BlacksmithRngState } {
    let t = (state.s + 0x6d2b79f5) >>> 0;
    const nextState = { s: t };
    t = Math.imul(t ^ (t >>> 15), t | 1);
    t ^= t + Math.imul(t ^ (t >>> 7), t | 61);
    const value = ((t ^ (t >>> 14)) >>> 0) / 4294967296;
    return { value, state: nextState };
}

/** Returns an int in [0, n) plus the advanced state. Pure. */
export function nextInt(state: BlacksmithRngState, n: number): { value: number; state: BlacksmithRngState } {
    const { value, state: next } = nextFloat(state);
    return { value: Math.floor(value * n), state: next };
}
