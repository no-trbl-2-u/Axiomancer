...
export interface AuthoredThreatPhase {
    ...
    rungs?: number;
    /** Phase D6e (spec 33 §2) — the open stance check this phase telegraphs.
     ...
     *  independent (a boss may name one of each). Undefined = no check. Only
     *  resolved while the upgradeable-dice flag is on (`resolveStanceCheck`);
     *  inert otherwise, so authoring it never disturbs the flag-off baseline. */
