...
function detailCore(card: CombatCard, skill?: Skill): DetailCore {
    ...
    switch (c.kind) {
        ...
        case 'weaken': return { subtitle: `${Title} the enemy — its attacks hit softer.`, metaChip, outcomeLine: `Apply ${Title}.`, outcomeStats: [], stacksText: null, freeLine, powerLine: `◆ WITH A DIE: apply ${Title} — the foe's next hit is weakened, plus a small hit.`, readNote: `${Title} weakens the enemy's hit; pile on different controls to deny the turn outright.`, mathLine: `${Title} lowers the enemy's attack roll; combined controls past a threshold deny the turn (exact amount is engine-tuned).`, keywords };
        ...
        case 'siphon': return { subtitle: 'Siphon — heal for part of the damage you deal.', metaChip, outcomeLine: `${Title} ${c.siphonPct}%.`, outcomeStats: [{ label: 'LIFESTEAL', value: `${c.siphonPct}%` }], stacksText: null, freeLine, powerLine: `◆ WITH A DIE: a full hit (scales with your ${skill?.scalingStat ?? 'stat'}) that heals ${c.siphonPct}% of the damage dealt; ▲ read ×${READ_DAMAGE_MULT.advantage}, ▼ ×${READ_DAMAGE_MULT.disadvantage}.`, readNote: `The hit (and so the heal) scales with the read; the ${c.siphonPct}% rate is exact.`, mathLine: `Heal = ${c.siphonPct}% × (damage dealt this hit) — the hit is live, so no fixed heal number.`, keywords };
        case 'compound': return { subtitle: 'Compound — punishes a debuff-laden foe.', metaChip, outcomeLine: `${Title} ${c.compoundPer}/debuff · max ${COMPOUND_COUNT_CAP}.`, outcomeStats: [{ label: 'PER DEBUFF', value: `${c.compoundPer}` }, { label: 'CAP', value: `${COMPOUND_COUNT_CAP}` }], stacksText: null, freeLine, powerLine: `◆ WITH A DIE: base hit + ${c.compoundPer} × (distinct debuffs, max ${COMPOUND_COUNT_CAP}); the read scales it.`, readNote: 'Stack more DISTINCT debuffs first to maximise the bonus.', mathLine: `bonus = ${c.compoundPer} × debuff count (live, capped ${COMPOUND_COUNT_CAP}) × read — count is live, so no fixed number.`, keywords };
        case 'execute': { const frac = Math.round(EXECUTE_DAMAGE_FRACTION * 100); return { subtitle: 'Execute — a finisher on a low or DoT-stacked foe.', metaChip, outcomeLine: `${Title} ${frac}% max HP · foe <${c.executeHpPct}% HP or ≥${c.executeStacks} DoT.`, outcomeStats: [{ label: 'TRIGGER', value: `< ${c.executeHpPct}% HP` }, { label: 'OR DOT', value: `≥ ${c.executeStacks}` }, { label: 'HIT', value: `${frac}% max HP` }], stacksText: c.riders.length ? 'Also applies its on-hit effects.' : null, freeLine, powerLine: `◆ WITH A DIE: if the foe is < ${c.executeHpPct}% HP or ≥${c.executeStacks} DoT stacks, Execute for ${frac}% of its max HP; otherwise its on-hit effects still land.`, readNote: 'Set it up with DoT, then fire when the foe is low or heavily stacked.', mathLine: `Execute = ${frac}% of the foe's MAX HP when armed (foe < ${c.executeHpPct}% HP OR ≥${c.executeStacks} DoT stacks).`, keywords }; }
        case 'rupture': return { subtitle: "Rupture — detonate the foe's damage-over-time.", metaChip, outcomeLine: `${Title} · max ${RUPTURE_BURST_CAP}.`, outcomeStats: [{ label: 'BURST', value: 'live total' }, { label: 'CAP', value: `${RUPTURE_BURST_CAP}` }], stacksText: null, freeLine, powerLine: `◆ WITH A DIE: detonate the foe's pending DoT for a burst (up to ${RUPTURE_BURST_CAP}); the read scales it.`, readNote: 'Stack DoT first — the burst equals the pending DoT, so it has no fixed number until you fire it.', mathLine: `Burst = pending DoT × read (capped ${RUPTURE_BURST_CAP}) — pending DoT is live, so no fixed number (real-units-or-no-number).`, keywords };
        case 'inert':
        default: return { subtitle: `${Title || 'Effect'} — minor right now.`, metaChip, outcomeLine: `${Title || 'This effect'} — minor for now.`, outcomeStats: [], stacksText: null, freeLine, powerLine: `◆ WITH A DIE: apply ${Title || 'the effect'}, plus a small hit.`, readNote: 'The read scales only the small strike.', mathLine: `${Title || 'This effect'} is not read by the live HP engine yet — no number shown.`, keywords };
    }
}
