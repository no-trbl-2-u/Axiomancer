 ...
 * render-layer logic the engine test (rest.tutorial.engine) does not
 * reach: the `index < 0` null-return gate, the `THE FIRST NIGHT ·
...
import { REST_TUTORIAL_STEPS } from '@/components/rest/tutorial-steps';
 ...
 */
function sessionAtStep(completed: number): RestSession {
    const phase = completed >= 5 ? 'done' : completed >= 4 ? 'outcome' : completed >= 1 ? 'watch' : 'posture';
    const watch = completed >= 3 ? 3 : completed >= 2 ? 2 : completed >= 1 ? 1 : 0;
    return { phase, watch } as unknown as RestSession;
}
...
describe('TutorialCoach', () => {
    describe('null-return gate', () => {
        it('renders nothing once the tutorial script is complete', () => {
            ...
            expect(queryByTestId('rest-tutorial')).toBeNull();
        });
        ...
        it('renders the coach banner while a step is still unmet', () => {
            ...
            expect(getByTestId('rest-tutorial')).toBeTruthy();
        });
    });
    ...
    describe('step counter', () => {
        it('shows 1 / total on the first step', () => {
            ...
            expect(getByText(`THE FIRST NIGHT · 1 / ${TOTAL}`)).toBeTruthy();
        });
    ...
    describe('current-step copy', () => {
        ...
        it('swaps to the next step copy when an earlier predicate is met', () => {
            const next = REST_TUTORIAL_STEPS[1];
            ...
            expect(queryByText(REST_TUTORIAL_STEPS[0].title)).toBeNull();
        });
    });
    ...
    describe('skip wiring', () => {
        it('fires onSkip when the SKIP control is pressed', () => {
            ...
            fireEvent.press(getByTestId('rest-tutorial-skip'));
