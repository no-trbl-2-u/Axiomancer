...
- **Unattended (the autonomous loop, e.g. `content-curator` inside
  ...
  alternatives internally against `character-references.md` prior art
  and spec 34 §2.5 (the delivery register), pick the strongest option,
  note the call in one line under a "Decided unattended" heading in the
  ...
  full authority to author and ship named characters without a per-item
